...
pub trait KeypairProvider: Sync + Send {
    ...
    fn sign(&self, cx: &mut Context<'_>, sign_data: &[u8]) -> CancelablePoll<io::Result<Vec<u8>>>;
}
...
pub trait ConnPool: Sync + Send {
    ...
    fn put(&self, cx: &mut Context<'_>, conn: P2pConn) -> CancelablePoll<io::Result<()>>;
    ...
    fn remove(&self, cx: &mut Context<'_>, conn: P2pConn) -> CancelablePoll<io::Result<()>>;
}
